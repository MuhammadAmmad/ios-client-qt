        ...
        [afn getDataFromPath:FILE_GET_ESTIMATE_FARE withParamData:dictParam withBlock:^(id response, NSError *error)
         {
             ...
             if (response)
             {
                 if([[response valueForKey:@"success"] boolValue])
                 {
                     ...
                     [PREF synchronize];
                     self.lMinFare.text= [NSString stringWithFormat:@"UGX %.2f",[[response valueForKey:@"eta"] floatValue]];
                 }
                 else
                 {
                     if ([[response valueForKey:@"error_code"] integerValue]==406)
...
-(NSArray*) calculateRoutesFrom:(CLLocationCoordinate2D) f to: (CLLocationCoordinate2D) t
{
    ...
    else
    {
        ...
        // APP SIDE CALCULATION FOR MIN FARE
        ...
        [PREF synchronize];
        self.lMinFare.text= [NSString stringWithFormat:@"UGX%.2f",[fare floatValue]];
         */
        self.tableforCityFare.hidden=YES;
        self.getFareView.hidden=YES;
...
-(void)carouselDidEndScrollingAnimation:(iCarousel *)carousel
{
    ...
    if (arrForApplicationType.count > 0 )
    {
        ...
        if (![self.lblServiceType.text isEqualToString:name])
        {
            ...
            self.lblMin_fare.text=[NSString stringWithFormat:@"UGX %@",[dict valueForKey:@"min_fare"]];
            self.lblPerKm.text=[NSString stringWithFormat:@"UGX %.2f per %@",[[dict valueForKey:@"price_per_unit_distance"] floatValue],[dict valueForKey:@"unit"]];
            self.lblTimeCost.text=[NSString stringWithFormat:@"UGX %.2f per min",[[dict valueForKey:@"price_per_unit_time"] floatValue]];
